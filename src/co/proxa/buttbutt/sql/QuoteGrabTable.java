...
public class QuoteGrabTable {
    ...
    public QuoteGrabTable(Buttbutt butt) {
        this.butt = butt;
    }
    ...
    public void addQuote(String playerName, String quote, String grabber) {
        if (butt.getSqlManager().isConnected()) {
            ...
            String update = "INSERT INTO `" + butt.getSqlManager().getTablePrefix() + "_quotes` (user,quote,grabbed_by,timestamp) VALUES(?,?,?,?)";
    ...
    public boolean quoteAlreadyExists(String playerName, String quote) throws SQLException{
        if (butt.getSqlManager().isConnected()) {
            String query = "SELECT * FROM `" + butt.getSqlManager().getTablePrefix() + "_quotes` WHERE user=? AND quote=?";
    ...
    public String getRandomQuote() {
        if (butt.getSqlManager().isConnected()) {
            String query = "SELECT * FROM `" + butt.getSqlManager().getTablePrefix() + "_quotes` ORDER BY RAND() LIMIT 1";
            ...
            try {
                ...
                butt.getLogger().severe("some kind of sql exception");
                ex.printStackTrace();
            }
        } else {
            butt.getSqlManager().reconnect();
    ...
    public String getQuoteById(int id) throws SQLException {
        if (butt.getSqlManager().isConnected()) {
            String query = "SELECT * FROM `" + butt.getSqlManager().getTablePrefix() + "_quotes` WHERE id=?";
            PreparedStatement ps = butt.getSqlManager().getConnection().prepareStatement(query);
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public String findQuote(String search) throws SQLException {
        if (butt.getSqlManager().isConnected()) {
            String query = "SELECT * FROM `" + butt.getSqlManager().getTablePrefix() + "_quotes` WHERE quote LIKE ?";
    ...
    public String getRandomQuoteFromPlayer(String username) throws SQLException {
        if (butt.getSqlManager().isConnected()) {
            String query = "SELECT * FROM `" + butt.getSqlManager().getTablePrefix() + "_quotes` WHERE user=? ORDER BY RAND() LIMIT 1";
            PreparedStatement ps = butt.getSqlManager().getConnection().prepareStatement(query);
    ...
    public String getLastQuoteFromPlayer(String username) throws SQLException {
        if (butt.getSqlManager().isConnected()) {
            String query = "SELECT * FROM `" +butt.getSqlManager().getTablePrefix() + "_quotes` WHERE user=? ORDER BY id DESC LIMIT 1";
            PreparedStatement ps = butt.getSqlManager().getConnection().prepareStatement(query);
            ps.setString(1, username);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
